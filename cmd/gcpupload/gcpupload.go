...
func main() {
	...
	var projectID, bucket, source, name, authfile string
	var public bool
	...
	flag.StringVar(&authfile, "authfile", "", "GCP auth file (JSON format)")
	...
	_, objAttrs, err := upload(ctx, r, projectID, bucket, name, authfile, public)
	if err != nil {
		switch err {
...
func upload(ctx context.Context, r io.Reader, projectID, bucket, name, authfile string, public bool) (*storage.ObjectHandle, *storage.ObjectAttrs, error) {
